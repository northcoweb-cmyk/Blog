...
const SECTION_TOPIC = {
  ...
  models: 'Neural network (machine learning)',
...
// Used when a story's own topic has no picture.
const BACKUP = ['Artificial intelligence', 'Data center', 'Semiconductor'];
...
export function topicFor(story) {
  ...
  for (const [re, title] of ENTITIES) if (re.test(story.source || '')) return title;
...
export async function attachTopicImages(stories) {
  const titles = [...new Set([...stories.map(topicFor), ...Object.values(SECTION_TOPIC), ...BACKUP])];
  const found = new Map(await Promise.all(titles.map(async (t) => [t, await wikiImage(t)])));
  for (const s of stories) {
    let img = found.get(topicFor(s));
    if (!img) img = [SECTION_TOPIC[s.section], ...BACKUP].map((t) => found.get(t)).find(Boolean) || null;
    if (!img) continue;
    if (!s.image) {
      s.image = img.url;
