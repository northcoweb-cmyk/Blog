...
/**
 * Today's post pack for posting by hand (no Meta connection needed):
 * top stories, max 2 per desk, each with its image, caption and a suggested time.
 */
export async function dailyPack({ origin, n = 6 }) {
  const settings = await getSettings();
  const news = await getNews();
  const perDesk = {};
  const picks = pickStories(news.stories, [], 40).filter((s) => {
    perDesk[s.section] = (perDesk[s.section] || 0) + 1;
    return perDesk[s.section] <= 2;
  }).slice(0, n);
  const span = (settings.endHour - settings.startHour) * 60;
  return picks.map((s, i) => {
    const mins = settings.startHour * 60 + Math.round((span / picks.length) * i);
    const h = Math.floor(mins / 60);
    const m = mins % 60 < 30 ? 0 : 30;
    return {
      id: s.id,
      title: s.title,
      source: s.source,
      section: s.section,
      card: cardUrl(origin, s, { style: settings.style }),
      caption: captionFor(s, origin),
