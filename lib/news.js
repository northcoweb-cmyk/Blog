...
async function buildNews() {
  ...
  for (const { src, items } of results) {
    for (const it of items) {
      ...
      const summary = it.summary && !/^(the post|appeared first)/i.test(it.summary) ? it.summary.replace(/\s*The post .* appeared first on .*$/i, '') : '';
  ...
  await enrichImages(stories.slice(0, 40));
...
async function enrichImages(stories) {
  const need = stories.filter((s) => !s.image && !/news\.google\.com/.test(s.url)).slice(0, 14);
  await Promise.all(
    need.map(async (s) => {
      try {
        ...
        if (img && isPublicHttpUrl(img)) s.image = img;
        if (!s.summary && meta['og:description']) s.summary = meta['og:description'].slice(0, 420);
      } catch {
        /* no image — the front end draws a branded cover instead */
      }
    }),
  );
}
