...
export const cronRoutes = {
  // Hourly from GitHub Actions (.github/workflows/instagram.yml). Needs CRON_SECRET.
  async instagram(req, res) {
    noStore(res);
    const secret = process.env.CRON_SECRET;
    if (!secret || (req.headers.authorization || '') !== `Bearer ${secret}`) return json(res, 401, { error: 'Unauthorized' });
    const { runAutopilot } = await import('../instagram.js');
    try {
      json(res, 200, await runAutopilot({ origin: siteOrigin(req) }));
    } catch (e) {
      json(res, 200, { posted: false, error: e.message });
    }
  },
